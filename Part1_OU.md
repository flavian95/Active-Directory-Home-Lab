...
![alt text](13.png)
The expanded `USERS` OU showing the `ADMIN`, `CONTRACTORS`, and `EMPLOYEES` sub‑OUs.

> 📸 **Screenshot:** `screenshots/12.png` – The complete OU tree with `_MANAGED`, its sub‑OUs, and the groups already created inside `_GROUPS`.  
> 📸 **Screenshot:** `screenshots/13.png` – The expanded `USERS` OU showing the `ADMIN`, `CONTRACTORS`, and `EMPLOYEES` sub‑OUs.
